...
@section('container')
    ...
    <div class="row justify-content-center mb-3">
        <div class="col-md-6">
            <form action="/posts">
                {{-- Menambahkan filter search (lihat di urlnya pasti ada & nya nah itu memfilter search&category) --}}
                @if (request('category'))
                    <input type="hidden" name="category" value="{{ request('category') }}">
                @endif
                {{-- Menambahkan filter search (lihat di urlnya pasti ada & nya nah itu memfilter search&author) --}}
                @if (request('author'))
